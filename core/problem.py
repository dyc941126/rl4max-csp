...
class Problem:
    ...
    def random_sensor_net(self, grid_size, domain_size, p2=-1, min_cost=0, max_cost=100, gc=False,
            ...
            for col in range(grid_size):
                ...
                if right in locations:
                    self.add_constraint([this_id, locations[right]], Problem._random_matrix(domain_size, domain_size,
                                                                           min_cost, max_cost, gc, weighted, decimal, p2=p2))
                if down in locations:
                    self.add_constraint([this_id, locations[down]], Problem._random_matrix(domain_size, domain_size,
                                                                           min_cost, max_cost, gc, weighted, decimal, p2=p2))

    def random_binary(self, nb_agent, domain_size, p1, p2=-1, min_cost=0, max_cost=100, gc=False,
                      weighted=False, decimal=-1):
    ...
    def random_scale_free(self, nb_agent, domain_size, m1, m2, p2=-1, min_cost=0, max_cost=100, gc=False,
                          weighted=False, decimal=-1):
        assert isinstance(nb_agent, int) and nb_agent > 0
    ...
    @classmethod
    def _random_matrix(cls, domain1, domain2, min_cost, max_cost, gc=False, weighted=False, decimal=-1, p2=-1):
        data = []
        for i in range(domain1):
            data.append([0] * domain2)
            for j in range(domain2):
                if 0 < p2 < 1:
                    cost = 1 if random.random() < p2 else 0
                elif not gc:
                    if decimal <= 0:
